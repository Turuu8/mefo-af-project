...
import { findUserByKey, updateUserByID } from "../utils/findDocument";
...
export const userCtrl = {
  createNewUser: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        msg: {
          en: "We sent an email verification link to your email. Please check it.",
          mn: "Бид таны мэйл хаягруу баталгаажуулах мэйл явуулсан. Та шалгана уу.",
        },
      });
    } catch (error) {
  ...
  forgotPassword: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        msg: { en: "We sent a reset password link to email. Please check it.", mn: "Бид таны мэйл хаягруу нууц үг солих линк явууллаа. Та шалгана уу." },
      });
    } catch (error) {
      return res.status(500).json({ msg: (error as Error).message });
    }
  ...
  resetPassword: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({ msg: { en: "Your password successfully changed.", mn: "Нууц үг амжилттай өөрчлөгдлөө." } });
    } catch (error) {
      return res.status(500).json({ msg: (error as Error).message });
    }
  },
